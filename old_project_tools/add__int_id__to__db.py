...
# offersId = []
# campaignIdList = [x['guid'] for x in db.campaign.find({"showConditions.retargeting": False}, {'guid': 1})]
# informerIdList = [x['guid'] for x in db.informer.find({}, {'guid': 1})]
# queri = {"campaignId": {"$in": campaignIdList}}
# for item in db.offer.find(queri, {"guid_int": 1, "_id": 0}):
#     offersId.append(item['guid_int'])
# db.stats_daily.rating.delete_many({'adv': {'$nin': informerIdList}})
# db.stats_daily.rating.delete_many({'full_rating': 0})
# db.stats_daily.rating.delete_many({'guid_int': {'$nin': offersId}})
...
#         print guid_int
#
cur = db['offer'].find({}, {'guid_int': 1, '_id': 0})
for doc in cur:
    d[doc['guid_int']] += 1
    if long(doc['guid_int']) >= 9223372036854775807:
        print doc['guid_int']
