...
for account_id in acc:
    ...
    for k, v in camp.iteritems():
        ...
        sheet.write(0, 1, 'count', style0)
        sheet.write(0, 2, 'cost', style0)
        ...
        order_buf = collections.OrderedDict(
            sorted(buffer.items(), key=lambda t: datetime.datetime.strptime(t[0], "%d.%m.%Y")))
        for key, value in order_buf.iteritems():
            sheet.write(count, 0, key, style1)
            sheet.write(count, 1, str(value[1]), style1)
            sheet.write(count, 2, str(value[0]), style1)
    ...
    ftp.close()
    link.append('/report/count_cost/%s.xls' % str(account_id))
